...
import static com.onnoeberhard.epotato.LocalDatabaseHandler.CONTACTS;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.CONTACT_NAME;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.DT;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.EPID;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.FEED_POTATOES;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.FOLLOWING;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.ID;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.NEW_FEED_POTATOES;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.NEW_POTATOES;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.NEW_POTATOES_PID;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.POTATO_FORM;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.POTATO_TEXT;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.RECEIVED_POTATOES;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.RECEIVED_POTATOES_TS;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.SENT_POTATOES;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.SENT_POTATOES_CODE;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.SENT_POTATOES_CODE_ERROR;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.SENT_POTATOES_CODE_NULL;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.SENT_POTATOES_CODE_RECEIVED;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.SENT_POTATOES_CODE_SENDING;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.SENT_POTATOES_CODE_SENT;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.SENT_POTATOES_NAMES;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.SENT_POTATOES_TSS;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.SENT_POTATOES_UIDS;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.TEMP_CONTACTS;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.UID;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.deserialize;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.getNiceDate;
...
public class HomeFragment extends Fragment {
            ...
            public void run() {
                for (int i = layoutManager.findFirstCompletelyVisibleItemPosition(); i <= layoutManager.findLastCompletelyVisibleItemPosition(); i++)
                    if (i >= 0 && adapter.items.get(i).ptype == PotatoAdapter.PotatoItem.TYPE_POTATO)
                        ldb.delete(NEW_POTATOES, NEW_POTATOES_PID, ((PotatoAdapter.Potato) adapter.items.get(i)).pid);
    ...
    private class PotatoAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
        ...
        void update() {
            items.clear();
            ArrayList<Map<String, String>> potatoes = ldb.getAll(type == HOME_TYPE_RECEIVE ? RECEIVED_POTATOES : type == HOME_TYPE_FEED ? FEED_POTATOES : SENT_POTATOES);
        ...
        class Potato extends PotatoItem {
            ...
            Potato(Map<String, String> potato) {
                ...
                if (type == HOME_TYPE_RECEIVE) {
                    uid = potato.get(UID);
                    name = ldb.get(CONTACTS, UID, uid, CONTACT_NAME);
                    name = name == null ? ldb.get(CONTACTS, UID, uid, EPID) : name;
                    name = name == null ? ldb.get(TEMP_CONTACTS, UID, uid, EPID) : name;
                    name = name == null ? "" : name;
                    contact_name = ldb.get(TEMP_CONTACTS, UID, uid, EPID);
                    ts = Boolean.parseBoolean(potato.get(RECEIVED_POTATOES_TS));
                } else if (type == HOME_TYPE_FEED) {
                    uid = potato.get(UID);
                    name = ldb.get(CONTACTS, UID, uid, CONTACT_NAME);
                    name = name == null ? ldb.get(CONTACTS, UID, uid, EPID) : name;
                    name = name == null ? ldb.get(FOLLOWING, UID, uid, EPID) : name;
                    name = name == null ? ldb.get(TEMP_CONTACTS, UID, uid, EPID) : name;
                    name = name == null ? "" : name;
                } else {
                    status = Integer.parseInt(potato.get(SENT_POTATOES_CODE));
                    uids = deserialize(potato.get(SENT_POTATOES_UIDS));
                    tss = deserialize(potato.get(SENT_POTATOES_TSS));
                    names = deserialize(potato.get(SENT_POTATOES_NAMES));
                    sent = true;
                    for (String uid : uids) {
                        String _name = ldb.get(CONTACTS, UID, uid, CONTACT_NAME);
                        _name = _name == null ? ldb.get(CONTACTS, UID, uid, EPID) : _name;
                        _name = _name == null ? ldb.get(TEMP_CONTACTS, UID, uid, EPID) : _name;
                    ...
                    if (uids.size() == 1)
                        contact_name = ldb.get(TEMP_CONTACTS, UID, uids.get(0), EPID);
                    for (String pname : deserialize(potato.get(SENT_POTATOES_NAMES))) {
                        if (pname != null)
                            name += name.equals("") ? pname : ", " + pname;
                    }
                }
                text = potato.get(POTATO_TEXT);
                try {
                    date = getNiceDate(potato.get(DT), mainActivity);
                } catch (ParseException e) {
                    e.printStackTrace();
                }
                dt = potato.get(DT);
                form = Integer.parseInt(potato.get(POTATO_FORM));
                nieuw = type == HOME_TYPE_RECEIVE && ldb.get(NEW_POTATOES, NEW_POTATOES_PID, pid, ID) != null;
            }
        }
                            ...
                            public boolean onMenuItemClick(MenuItem menuItem) {
                                switch (menuItem.getItemId()) {
                                    ...
                                    case R.id.addcontact:
                                        ...
                                        break;
                                    case R.id.markread:
                                        ldb.delete(NEW_POTATOES, NEW_POTATOES_PID, p.pid);
                                        p.nieuw = false;
        ...
        class SmallViewHolder extends PotatoViewHolder {
            ...
            void bind(final PotatoAdapter.PotatoItem pi) {
                ...
                ll.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        if (p.sent && p.status != SENT_POTATOES_CODE_NULL) {
                            builder.setTitle(getString(R.string.status) + (p.status == SENT_POTATOES_CODE_SENDING ? getString(R.string.sending)
                                    : p.status == SENT_POTATOES_CODE_SENT ? getString(R.string.sent)
                                    : p.status == SENT_POTATOES_CODE_RECEIVED ? getString(R.string.received) : getString(R.string.errorSending)));
                        }
                        if (p.sent) {
                            final boolean addToContacts = p.uids.size() == 1 && ldb.get(CONTACTS, UID, p.uids.get(0), ID) == null && p.contact_name != null;
                            final boolean sendAgain = p.status == SENT_POTATOES_CODE_ERROR;
                            ...
                            final boolean following = ldb.get(FOLLOWING, UID, p.uid, ID) != null && ldb.get(CONTACTS, UID, p.uid, ID) == null;
                            ...
                            final boolean addToContacts = ldb.get(CONTACTS, UID, p.uid, ID) == null && p.contact_name != null;
            ...
            void updateStatus() {
                statusiv.setVisibility(p != null && p.sent && p.status != SENT_POTATOES_CODE_NULL ? View.VISIBLE : View.GONE);
                if (p != null && p.sent && p.status != SENT_POTATOES_CODE_NULL) {
                    statusiv.setImageResource(p.status == SENT_POTATOES_CODE_SENDING ? R.drawable.ic_query_builder_black_24dp
                            : p.status == SENT_POTATOES_CODE_SENT ? R.drawable.ic_done_black_24dp
                            : p.status == SENT_POTATOES_CODE_RECEIVED ? R.drawable.ic_done_all_black_24dp
    ...
    class PotatoViewHolder extends RecyclerView.ViewHolder {
        ...
        void unfollow() {
            ldb.delete(FOLLOWING, UID, p.uid);
        ...
        void delete() {
            ldb.delete(type == HOME_TYPE_RECEIVE ? RECEIVED_POTATOES : type == HOME_TYPE_FEED ? FEED_POTATOES : SENT_POTATOES, ID, p.pid);
            if (type == HOME_TYPE_RECEIVE)
                ldb.delete(NEW_POTATOES, NEW_POTATOES_PID, p.pid);
            else if (type == HOME_TYPE_FEED)
                ldb.delete(NEW_FEED_POTATOES, NEW_POTATOES_PID, p.pid);
            mainActivity.updateTabBadges();
