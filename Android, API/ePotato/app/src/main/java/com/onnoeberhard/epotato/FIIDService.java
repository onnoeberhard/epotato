...
import static com.onnoeberhard.epotato.EndpointsHandler.PROFILE;
import static com.onnoeberhard.epotato.EndpointsHandler.PR_FIIDS;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.CONTACTS;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.CONTACTS_SCORE;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.CREEPS;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.EPID;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.FEED_NOTIFICATIONS;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.FIID;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.FOLLOWERS;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.FOLLOWING;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.HI_SCORE;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.ID;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.PHONE;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.TOTAL_STRANGERS;
...
public class FIIDService extends FirebaseInstanceIdService {
    ...
    static void updateFIEPID(final Context c, final FIEPIDRequest request) {
        ...
        new EndpointsHandler(c).getProfile(new EndpointsHandler.APIRequest() {
            ...
            public void onResult(Object result) {
                if (result != null) {
                    ...
                    sp.edit().putString(EPID, p.getEpid())
                            .putString(PHONE, p.getPhone() == null || p.getPhone().equals(EndpointsHandler.NULL) ? null : p.getPhone())
                            .putInt(FOLLOWERS, p.getFollowers() != null ? p.getFollowers().size() : 0)
                            .putBoolean(TOTAL_STRANGERS, p.getStrangers()).apply();
                    ...
                    if (p.getFiids() == null || !p.getFiids().contains(fiid)) {
                        ...
                        fiids.remove(sp.getString(FIID, ""));
                        sp.edit().putString(FIID, fiid).apply();
                        fiids.add(fiid);
                        new EndpointsHandler(c).update(null, PROFILE, EndpointsHandler.ID, p.getId().toString(),
                                new ArrayList<>(Collections.singletonList(PR_FIIDS)), new ArrayList<>(Collections.singletonList(LocalDatabaseHandler.serialize(fiids))));
                    }
        ...
        }, EndpointsHandler.ID, Long.toString(sp.getLong(UID, 0L)));
    }
    ...
    static void analytics(Context c) {
        ...
        analytics.setUserProperty(CONTACTS, Integer.toString(ldb.getAll(CONTACTS, ID).size()));
        analytics.setUserProperty(CREEPS, Boolean.toString(sp.getBoolean(CREEPS, true)));
        analytics.setUserProperty(FEED_NOTIFICATIONS, Boolean.toString(sp.getBoolean(FEED_NOTIFICATIONS, true)));
        analytics.setUserProperty(FOLLOWERS, Integer.toString(sp.getInt(FOLLOWERS, 0)));
        analytics.setUserProperty(FOLLOWING, Integer.toString(ldb.getAll(FOLLOWING, ID).size()));
        ArrayList<String> contacts = ldb.getAll(CONTACTS, CONTACTS_SCORE);
        //noinspection SuspiciousMethodCalls
        contacts.removeAll(Collections.singleton(null));
        Collections.sort(contacts);
        analytics.setUserProperty(HI_SCORE, contacts.size() > 0 ? contacts.get(0) : "0");
        analytics.setUserProperty(PHONE, Boolean.toString(sp.getString(LocalDatabaseHandler.PHONE, null) != null));
        analytics.setUserProperty(TOTAL_STRANGERS, Boolean.toString(sp.getBoolean(TOTAL_STRANGERS, true)));
    }
