...
class EndpointsHandler extends AsyncTask<String, Void, Object> {
    ...
    void update(APIRequest request, String kind, String property, String value, ArrayList<String> properties, ArrayList<String> values) {
        this.request = request;
        String p = LocalDatabaseHandler.serialize(properties);
        String v = LocalDatabaseHandler.serialize(values);
    ...
    void sendPotato(APIRequest request, ArrayList<String> uids, ArrayList<String> ts, String message, int type, String pid) {
        ...
        execute(SEND_POTATO, message, Integer.toString(type), pid, LocalDatabaseHandler.serialize(uids), LocalDatabaseHandler.serialize(ts));
    }
    ...
    void getTS(APIRequest request, int n) {
        ...
        execute(GET_TS, Integer.toString(n), LocalDatabaseHandler.serialize(contacts));
    }
    ...
    void getContacts(APIRequest request, ArrayList<String> numbers) {
        this.request = request;
        execute(GET_CONTACTS, LocalDatabaseHandler.serialize(numbers));
    }
    ...
    protected Object doInBackground(String... params) {
        ...
        try {
            switch (params[0]) {
                ...
                case UPDATE:
                    return api.update(params[1], params[2], params[3], LocalDatabaseHandler.deserialize(params[4]), LocalDatabaseHandler.deserialize(params[5])).execute();
                case SMS:
                ...
                case SEND_POTATO:
                    ...
                    for (String _sts : LocalDatabaseHandler.deserialize(params[5]))
                        ts.add(Boolean.parseBoolean(_sts));
                    return api.sendPotato(LocalDatabaseHandler.deserialize(params[4]), ts, Long.toString(PreferenceManager.getDefaultSharedPreferences(context).getLong(LocalDatabaseHandler.UID, 0L)), Integer.parseInt(params[2]), params[1], params[3]).execute();
                case GET_TS:
                    return api.getTS(Integer.parseInt(params[1]), LocalDatabaseHandler.deserialize(params[2])).execute();
                case NEW_CONTACT:
                ...
                case GET_CONTACTS:
                    return api.getContacts(Long.toString(PreferenceManager.getDefaultSharedPreferences(context).getLong(LocalDatabaseHandler.UID, 0L)), LocalDatabaseHandler.deserialize(params[1])).execute();
