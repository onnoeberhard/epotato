...
import com.onnoeberhard.epotato.inappbilling.IabHelper;
import com.onnoeberhard.epotato.inappbilling.IabResult;
import com.onnoeberhard.epotato.inappbilling.Inventory;
...
import static com.onnoeberhard.epotato.LocalDatabaseHandler.FIRST_PHONE_CONTACTS_JOB;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.FOLLOWERS;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.OLD_UID;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.PASSWORD;
import static com.onnoeberhard.epotato.LocalDatabaseHandler.PREMIUM;
...
public class MainActivity extends AppCompatActivity {
    ...
    static void logout(Activity a) {
        ...
        sp.edit().putLong(OLD_UID, sp.getLong(UID, 0L)).remove(UID).remove(EPID).remove(PASSWORD).remove(FIID).remove(FIRST_PHONE_CONTACTS_JOB).remove(FOLLOWERS).apply();
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (!sp.contains(UID))
            gotoWelcome(this);
        else if (sp.contains(OLD_UID) && sp.getLong(OLD_UID, 0L) != sp.getLong(UID, 0L)) {
            new AlertDialog.Builder(this).setTitle(R.string.newUser)
                    ...
                    .setPositiveButton(R.string.cont, new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            sp.edit().remove(OLD_UID).remove(FIRST_PHONE_CONTACTS_JOB).apply();
                            gotoMain();
                    ...
                    .setNegativeButton(R.string.logout, new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                            sp.edit().remove(UID).remove(EPID).remove(PASSWORD).remove(FIID).apply();
    ...
    private void gotoMain() {
        ...
        if (!sp.contains(FIRST_PHONE_CONTACTS_JOB))
        ...
        }, Long.toString(sp.getLong(UID, 0L)), sp.getString(PASSWORD, null));
                    ...
                    try {
                        iabHelper.queryInventoryAsync(new IabHelper.QueryInventoryFinishedListener() {
                            ...
                            public void onQueryInventoryFinished(IabResult result, Inventory inv) {
                                if (result.isSuccess())
                                    sp.edit().putBoolean(PREMIUM, inv.hasPurchase(PREMIUM)).apply();
                            }
                        });
